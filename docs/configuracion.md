...
**Cómo funciona:**
1. Se detecta un impacto cuando se cumplen **las tres** condiciones: la aceleración total supera **6G**, la componente **horizontal** de ese pico supera **2.5G**, y la velocidad venía siendo mayor a **20 km/h** en los 5 segundos anteriores. La exigencia horizontal es la que distingue un choque (convierte la velocidad en deceleración hacia adelante o al costado) de un golpe del camino: un **resalto o un hueco tomado rápido es un pico casi puramente vertical** y ya no dispara nada. Por encima de **12G** se acepta el impacto sin exigir componente horizontal, porque ningún resalto explica ese pico.
2. Tras el impacto, la velocidad debe **colapsar a inmovilidad dentro de 8 segundos** — un choque real detiene la moto de inmediato. Si sigue rodando pasado ese plazo, se descarta el impacto: una parada posterior (semáforo, parqueo) ya no se reinterpreta como el desenlace de ese golpe.
3. Si el vehículo queda inmóvil (velocidad menor a 3 km/h y aceleración por debajo de 1.3G) durante **30 segundos sostenidos**, se dispara la alarma. Si detecta movimiento normal después del golpe (por ejemplo, seguiste rodando a más de 10 km/h), lo interpreta como falso positivo y vuelve a vigilar sin avisar nada.
4. Se muestra una notificación de pantalla completa con alarma sonora y una cuenta regresiva de **60 segundos** con el botón **"ESTOY BIEN"**. Además, la cuenta regresiva **se anuncia por voz y vibración** cada 10 segundos: la voz sale por el stream de media, que es el que llega al **intercomunicador del casco** por Bluetooth, para que puedas cancelarla con el celular en el bolsillo. Ese aviso hablado **ignora a propósito** el interruptor maestro de alertas y las categorías de voz — es el único canal por el que te enterás mientras rodás.
5. Si nadie responde a tiempo, se envía un **SMS** (partido en varios mensajes si es largo) al contacto de emergencia con tu última ubicación conocida: *"RevScope: posible caida detectada (‹vehículo›). Ubicacion: https://maps.google.com/?q=lat,lon"*.
